...
const Cart = {
  addToCart: (cart, result) => {
    const sql = "SELECT * FROM product WHERE id = ? AND status = 1";
    ...
    db.query(sql, [cart.product_id], (err, data) => {
      ...
      db.query(
        insertSql,
        [cart.product_id, cart.qty, cart.qty],
        (err, insertResult) => {
          if (err) {
  ...
  products.image
FROM cart AS cart
JOIN product AS products ON cart.product_id = products.id;
  ...
  delete: (product_id, mycallback) => {
    const sql = `DELETE FROM cart WHERE product_id = ?`; // Use ? to avoid SQL injection
  ...
  update: (cart, product_id, mycallback) => {
    var sql = `UPDATE cart SET ? WHERE product_id = ?`;
